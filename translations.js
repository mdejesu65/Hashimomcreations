// translations.js - Complete bilingual translations for HashiMomCreations
const translations = {
  en: {
    // Cookie Banner
    cookie_text: "We use cookies to ensure the best experience 🍪",
    cookie_accept: "Accept",
    
    // Hero Section
    hero_title: "HashiMomCreations",
    hero_subtitle: "Helping Moms Turn Wellness & Wisdom into Beautiful, Profitable Brands",
    hero_cta: "Start Your Website",
    
    // About Section
    about_title: "About Me",
    about_p1: "I'm <strong>Maciel DeJesus</strong>. When Hashimoto's forced me to rebuild my life, I discovered something: <strong>the tech skills I learned while brain-fogged became my superpower.",
    about_p2: "Now I build websites for moms like us - the ones juggling autoimmune flares, kids, and big dreams. <strong>I know you don't have energy to waste.</strong> That's why I handle all the tech while you focus on healing and serving your clients.",
    about_p3: "Your illness isn't your identity. It's your strength. Let me build the platform that turns your wellness journey into your family's financial freedom 🌿✨",
    
    // Features Section
    features_title: "What You'll Find Here",
    feature1_title: "Wellness Tips",
    feature1_text: "Simple tools for healing, hormones, and energy.",
    feature2_title: "Freebies",
    feature2_text: "Printable guides, checklists, and starter kits made with love.",
    feature3_title: "Website Help",
    feature3_text: "Beautiful, simple websites to launch your brand or biz.",
    
    // Testimonials Section
    testimonials_title: "What Other Moms Are Saying",
    testimonial1: '"HashiMomCreations made me feel seen. I finally understand how to support my body!"',
    testimonial1_name: "— Laura, Mom of 3",
    testimonial2: '"The starter kit helped me launch my own wellness blog. I feel so confident now!"',
    testimonial2_name: "— Gaby, New Wellness Coach",
    
    // Services Section
    services_title: "What I Offer",
    service1_title: "✨ Custom Websites",
    service1_text: "Unique, feminine, mobile-friendly designs to match your story and mission.",
    service2_title: "🧩 Templates & Branding",
    service2_text: "Starter kits and Canva designs to make launching your brand easier than ever.",
    service3_title: "🤝 1-on-1 Tech Help",
    service3_text: 'I\'ll walk with you every step of the way — even if you\'re not "techy."',
    
    // Navigation - MOVED INSIDE en object
    nav_about: "About",
    nav_portfolio: "Portfolio",
    nav_services: "Services",
    nav_pricing: "Investment",
    nav_contact: "Book a Call",
    
    // Portfolio Section - MOVED INSIDE en object
    portfolio_title: "Recent Projects",
    portfolio_glowtrade_title: "GlowTrade AI",
    portfolio_glowtrade_desc: "Built a modern AI technology website with seamless user experience",
    portfolio_conde_title: "Conde de St Germain Technology LLC",
    portfolio_conde_desc: "Enterprise technology website with custom integrations",
    portfolio_view: "View Live Site →",
    portfolio_launching: "Launching January 2025",
    
    // Pricing Section - MOVED INSIDE en object
    pricing_title: "Investment",
    pricing_starter_title: "Website Starter",
    pricing_starter_price: "Starting at $497",
    pricing_builder_title: "Brand Builder",
    pricing_builder_price: "Starting at $997",
    pricing_launch_title: "Full Launch",
    pricing_launch_price: "Starting at $1,497",
    pricing_popular: "Most Popular",
    pricing_note: "Payment plans available • Book a call for custom projects",
    
    // Freebie Section
    freebie_title: "Free Gift for You 💌",
    freebie_text: "Download my free Website Starter Kit for Moms — and start building your brand today!",
    freebie_btn: "Download the Kit",
    
    // Work Section
    work_title: "How We Can Work Together",
    work_step1: "<strong>Book a Free 15-min Call</strong> – Let's chat and vibe!",
    work_step2: "<strong>Pick Your Package</strong> – Custom, template, or coaching",
    work_step3: "<strong>We Start!</strong> – I'll build your dream site 💻✨",
    work_btn: "Schedule Now",
    
    // Instagram Section
    ig_title: "Follow Me on Instagram",
    ig_text: 'Let\'s grow together at <strong>@hashimomcreations</strong> — where healing meets hustle 💫',
    
    // Contact Section
    contact_title: "Let's Connect",
    contact_text: "Ready to bring your vision to life? Book your discovery call below.",
    contact_email: "hashimomcreations@gmail.com",
    
    // CTA Section
    cta_section_title: "Ready to Start Your Journey?",
    cta_section_text: "Grab your FREE Website Starter Kit and begin building your dream.",
    cta_section_btn: "Yes, Send It to Me!",
    
    // Footer
    footer_legal: "© 2025 HashiMomCreations. All Rights Reserved.",
    footer_credit: 'A brand proudly created by <strong>GrowEasy Digital</strong>.',
    footer_terms: "Terms of Use",
    footer_privacy: "Privacy Policy",
    footer_license: "MIT License"
  },
  
  es: {
    // Cookie Banner
    cookie_text: "Usamos cookies para asegurar la mejor experiencia 🍪",
    cookie_accept: "Aceptar",
    
    // Hero Section
    hero_title: "HashiMomCreations",
    hero_subtitle: "Ayudando a Mamás a Convertir Bienestar y Sabiduría en Marcas Hermosas y Rentables",
    hero_cta: "Comienza Tu Sitio Web",
    
    // About Section
    about_title: "Por Qué Te Entiendo",
    about_p1: "Soy <strong>Maciel DeJesus</strong>. Cuando Hashimoto me obligó a reconstruir mi vida, descubrí algo: <strong>las habilidades técnicas que aprendí con niebla mental se convirtieron en mi superpoder.</strong>",
    about_p2: "Ahora construyo sitios web para mamás como nosotras - las que hacen malabares con brotes autoinmunes, niños y grandes sueños. <strong>Sé que no tienes energía para desperdiciar.</strong> Por eso manejo toda la tecnología mientras tú te enfocas en sanar y servir a tus clientes.",
    about_p3: "Tu enfermedad no es tu identidad. Es tu fortaleza. Déjame construir la plataforma que convierta tu viaje de bienestar en la libertad financiera de tu familia.",
    
    // Features Section
    features_title: "Lo Que Encontrarás Aquí",
    feature1_title: "Consejos de Bienestar",
    feature1_text: "Herramientas simples para sanar, equilibrar hormonas y tener más energía.",
    feature2_title: "Regalos Digitales",
    feature2_text: "Guías imprimibles, listas de verificación y kits de inicio hechos con amor.",
    feature3_title: "Ayuda con tu Web",
    feature3_text: "Sitios web simples y bellos para lanzar tu marca o negocio.",
    
    // Testimonials Section
    testimonials_title: "Lo Que Dicen Otras Mamás",
    testimonial1: '"HashiMomCreations me hizo sentir comprendida. ¡Por fin entiendo cómo apoyar mi cuerpo!"',
    testimonial1_name: "— Laura, Mamá de 3",
    testimonial2: '"El kit de inicio me ayudó a lanzar mi propio blog de bienestar. ¡Ahora me siento muy segura!"',
    testimonial2_name: "— Gaby, Nueva Coach de Bienestar",
    
    // Services Section
    services_title: "Lo Que Ofrezco",
    service1_title: "✨ Sitios Web Personalizados",
    service1_text: "Diseños únicos, femeninos y adaptables que reflejan tu historia y misión.",
    service2_title: "🧩 Plantillas y Marca",
    service2_text: "Kits iniciales y diseños en Canva para facilitar el lanzamiento de tu marca.",
    service3_title: "🤝 Ayuda Técnica 1-a-1",
    service3_text: 'Te acompañaré en cada paso, incluso si no eres experta en tecnología.',
    
    // Navigation - MOVED INSIDE es object
    nav_about: "Sobre Mí",
    nav_portfolio: "Portafolio",
    nav_services: "Servicios",
    nav_pricing: "Inversión",
    nav_contact: "Agenda una Llamada",
    
    // Portfolio Section - MOVED INSIDE es object
    portfolio_title: "Proyectos Recientes",
    portfolio_glowtrade_title: "GlowTrade AI",
    portfolio_glowtrade_desc: "Construí un sitio web moderno de tecnología AI con experiencia de usuario perfecta",
    portfolio_conde_title: "Conde de St Germain Technology LLC",
    portfolio_conde_desc: "Sitio web de tecnología empresarial con integraciones personalizadas",
    portfolio_view: "Ver Sitio en Vivo →",
    portfolio_launching: "Lanzamiento Enero 2025",
    
    // Pricing Section - MOVED INSIDE es object
    pricing_title: "Inversión",
    pricing_starter_title: "Sitio Web Inicial",
    pricing_starter_price: "Desde $497",
    pricing_builder_title: "Constructor de Marca",
    pricing_builder_price: "Desde $997",
    pricing_launch_title: "Lanzamiento Completo",
    pricing_launch_price: "Desde $1,497",
    pricing_popular: "Más Popular",
    pricing_note: "Planes de pago disponibles • Agenda una llamada para proyectos personalizados",
    
    // Freebie Section
    freebie_title: "Un Regalo Para Ti 💌",
    freebie_text: "Descarga gratis mi Kit de Inicio para Mamás — ¡y empieza a construir tu marca hoy!",
    freebie_btn: "Descargar el Kit",
    
    // Work Section
    work_title: "Cómo Podemos Trabajar Juntas",
    work_step1: "<strong>Agenda una llamada gratuita de 15 minutos</strong> – ¡Conversemos y conectemos!",
    work_step2: "<strong>Elige tu paquete</strong> – Personalizado, plantilla o coaching",
    work_step3: "<strong>¡Empezamos!</strong> – Yo construiré el sitio de tus sueños 💻✨",
    work_btn: "Agendar Ahora",
    
    // Instagram Section
    ig_title: "Sígueme en Instagram",
    ig_text: 'Crezcamos juntas en <strong>@hashimomcreations</strong> — donde sanar se encuentra con emprender 💫',
    
    // Contact Section
    contact_title: "Conectemos",
    contact_text: "¿Lista para darle vida a tu visión? Agenda tu llamada de descubrimiento abajo.",
    contact_email: "hashimomcreations@gmail.com",
    
    // CTA Section
    cta_section_title: "¿Lista Para Comenzar Tu Camino?",
    cta_section_text: "Descarga tu Kit de Inicio GRATIS y comienza a construir tu sueño.",
    cta_section_btn: "¡Sí, Envíamelo!",
    
    // Footer
    footer_legal: "© 2025 HashiMomCreations. Todos los Derechos Reservados.",
    footer_credit: 'Una marca creada con orgullo por <strong>GrowEasy Digital</strong>.',
    footer_terms: "Términos de Uso",
    footer_privacy: "Política de Privacidad",
    footer_license: "Licencia MIT"
  }
};